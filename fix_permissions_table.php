...
// Verifica se la tabella permissions esiste
...
if ($table_result->num_rows == 0) {
    ...
    if ($conn->query($create_table) === TRUE) {
        $message .= "Tabella permissions creata con successo.<br>";
    } else {
        $message .= "Errore nella creazione della tabella: " . $conn->error . "<br>";
    }
} else {
    $message .= "La tabella permissions esiste già.<br>";
}
...
$check_column = "SHOW COLUMNS FROM permissions LIKE 'duration'";
$column_result = $conn->query($check_column);
if ($column_result->num_rows == 0) {
    $add_column = "ALTER TABLE permissions ADD COLUMN duration VARCHAR(100) DEFAULT 'Non specificato'";
    if ($conn->query($add_column) === TRUE) {
...
$check_column = "SHOW COLUMNS FROM permissions LIKE 'manual_username'";
$column_result = $conn->query($check_column);
if ($column_result->num_rows == 0) {
    $add_column = "ALTER TABLE permissions ADD COLUMN manual_username VARCHAR(100) NULL AFTER user_id";
    if ($conn->query($add_column) === TRUE) {
...
<head>
    ...
    <title>Diario - Fix Tabella Permessi</title>
...
<body>
    ...
    <div class="main-content">
        <h1 class="page-title"><i class="fas fa-wrench"></i> Fix Tabella Permessi</h1>
        ...
        <div class="action-buttons">
            <a href="permissions.php" class="btn"><i class="fas fa-arrow-left"></i> Torna ai Permessi</a>
        </div>
    </div>
</body>
</html>